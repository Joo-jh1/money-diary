  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#FF6B6B 0%,#FF8E53 40%,#FFD93D 100%)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24 }}>
      ...
      <div style={{ width:"100%", maxWidth:400, background:"white", borderRadius:32, padding:"36px 28px 32px", boxShadow:"0 20px 60px rgba(0,0,0,0.15)", animation:"slideUp .5s ease" }}>
        ...
        <input placeholder="예: 소희💰  (최대 10자)" maxLength={10} value={nick} onChange={e => setNick(e.target.value)}
        ...
        <input placeholder="예: 용돈 아끼고 닌텐도 살 거야 🎮  (최대 30자)" maxLength={30} value={bio} onChange={e => setBio(e.target.value)}
    ...
    <div style={{ background:"linear-gradient(135deg,#FF6B6B,#FF8E53)", borderRadius:24, padding:"20px 20px 18px", marginBottom:16, color:"white", position:"relative", overflow:"hidden", boxShadow:"0 8px 28px rgba(255,107,107,0.32)" }}>
      ...
      <div style={{ marginBottom:16, position:"relative", zIndex:10 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:4 }}>
          ...
          <button onClick={onEdit} style={{ background:"rgba(255,255,255,.28)", border:"2px solid rgba(255,255,255,.5)", color:"white", borderRadius:12, padding:"8px 18px", fontSize:14, cursor:"pointer", fontWeight:800, touchAction:"manipulation", userSelect:"none", WebkitUserSelect:"none", zIndex:20, position:"relative" }}>✏️ 설정</button>
        </div>
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.55)", zIndex:300, display:"flex", alignItems:"center", justifyContent:"center", backdropFilter:"blur(6px)", padding:24 }}>
      <div style={{ background:"white", borderRadius:28, padding:28, width:"100%", maxWidth:360, animation:"slideUp .3s ease" }}>
        ...
        <div style={{ display:"flex", gap:10 }}>
          ...
          <button onClick={() => onSave({ cash: Number(cash)||0, card: Number(card)||0 })} style={{ flex:2, padding:14, borderRadius:16, border:"none", background:"linear-gradient(135deg,#FF6B6B,#FF8E53)", color:"white", fontSize:14, fontWeight:900, cursor:"pointer", boxShadow:"0 4px 14px rgba(255,107,107,.35)" }}>저장하기 ✓</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.55)", zIndex:300, display:"flex", alignItems:"flex-end", backdropFilter:"blur(6px)" }}>
      <div style={{ background:"white", width:"100%", maxWidth:430, margin:"0 auto", borderRadius:"28px 28px 0 0", padding:"20px 20px 48px", maxHeight:"85vh", overflowY:"auto", overscrollBehavior:"contain", touchAction:"pan-y", animation:"modalUp .35s cubic-bezier(.34,1.56,.64,1)" }}>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:14 }}>
          {QUICK.map(q => (
            <button key={q} onClick={() => setMemo(q)} style={{ padding:"8px 14px", borderRadius:20, border:`2px solid ${memo===q?"#66BB6A":"#eee"}`, background:memo===q?"#E8F5E9":"white", color:memo===q?"#2E7D32":"#888", fontSize:13, fontWeight:700, cursor:"pointer", transition:"all .2s" }}>{q}</button>
          ))}
        </div>
        ...
        <div style={{ display:"flex", gap:10, marginBottom:20 }}>
          <button onClick={() => setPay("cash")} style={{ flex:1, padding:"12px 0", borderRadius:16, border:`2px solid ${payMethod==="cash"?"#66BB6A":"#eee"}`, background:payMethod==="cash"?"#f0fff4":"white", color:payMethod==="cash"?"#2E7D32":"#aaa", fontSize:14, fontWeight:800, cursor:"pointer", transition:"all .2s" }}>💵 현금</button>
          <button onClick={() => setPay("card")} style={{ flex:1, padding:"12px 0", borderRadius:16, border:`2px solid ${payMethod==="card"?"#45B7D1":"#eee"}`, background:payMethod==="card"?"#f0f9ff":"white", color:payMethod==="card"?"#45B7D1":"#aaa", fontSize:14, fontWeight:800, cursor:"pointer", transition:"all .2s" }}>💳 카드충전</button>
        </div>
    ...
    <div style={{ background:"white", borderRadius:22, overflow:"hidden", boxShadow:"0 2px 16px rgba(0,0,0,.07)", marginBottom:18, animation:"slideUp .4s ease", width:"100%" }}>
      ...
      <div onClick={() => onComment && onComment(post)} style={{ padding:"12px 14px", display:"flex", alignItems:"center", gap:8, cursor:"pointer", width:"100%", boxSizing:"border-box" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:4, flexShrink:0 }}>
          ...
          {post.payMethod === "card"
            ? <div style={{ background:"#e3f6fd", color:"#45B7D1", padding:"3px 6px", borderRadius:10, fontSize:10, fontWeight:800 }}>💳</div>
            : <div style={{ background:"#f0f0f0", color:"#aaa", padding:"3px 6px", borderRadius:10, fontSize:10, fontWeight:800 }}>💵</div>
          }
      ...
      {menu && (
        <div style={{ margin:"0 14px 10px", background:"#f8f8f8", borderRadius:14, border:"1px solid #eee", overflow:"hidden" }}>
      ...
      <div style={{ padding: post.imageUrl ? "11px 14px 6px" : "4px 14px 6px", boxSizing:"border-box" }}>
        {!post.imageUrl && (
          <div onDoubleClick={doubleTap} style={{ display:"flex", alignItems:"center", gap:10, background:`${c.color}0e`, borderRadius:14, padding:"12px 14px", marginBottom:10, cursor:"pointer" }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              {post.memo
                ? <div style={{ fontSize:13, color:"#333", fontWeight:600, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{post.memo}</div>
                : <div style={{ fontSize:13, color:"#bbb" }}>기록 없음</div>
              }
            </div>
            <div style={{ background:`linear-gradient(135deg,${c.color},${c.color}bb)`, color:"white", padding:"6px 12px", borderRadius:16, fontSize:14, fontWeight:900, flexShrink:0 }}>
              -{won(post.amount)}
            </div>
          </div>
        )}
        ...
        {post.imageUrl && (
          <>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <button onClick={() => { const n = !liked; setLiked(n); if (n) onLike(post.id); }} style={{ background:"none", border:"none", cursor:"pointer", fontSize:24, padding:0, transition:"transform .2s", transform: liked ? "scale(1.2)" : "scale(1)", flexShrink:0 }}>
                {liked ? "❤️" : "🤍"}
              </button>
              {post.sticker && <span style={{ fontSize:20 }}>{post.sticker}</span>}
              <div style={{ marginLeft:"auto", background:`linear-gradient(135deg,${c.color},${c.color}bb)`, color:"white", padding:"6px 14px", borderRadius:20, fontSize:15, fontWeight:900, boxShadow:`0 3px 12px ${c.color}44`, flexShrink:0 }}>
                -{won(post.amount)}
              </div>
            </div>
            {post.memo && (
              <div style={{ marginTop:10, fontSize:14, color:"#333", lineHeight:1.6, overflow:"hidden" }}>
    ...
    <div style={{ position:"fixed", inset:0, background:"white", zIndex:300, display:"flex", flexDirection:"column", overflow:"hidden" }}>
      ...
      <div style={{ flex:1, overflowY:"auto", overscrollBehavior:"contain", touchAction:"pan-y", padding:"16px 20px 32px", WebkitOverflowScrolling:"touch" }}>
        <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:18, background:"#f8f8f8", borderRadius:16, padding:12 }}>
          <div onClick={() => fileRef.current.click()} style={{ position:"relative", cursor:"pointer", flexShrink:0 }}>
            <div style={{ width:80, height:80, borderRadius:14, overflow:"hidden", background:`${cc.color}22`, display:"flex", alignItems:"center", justifyContent:"center" }}>
              {img ? <img src={img} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }} /> : <span style={{ fontSize:36 }}>{sticker || cc.emoji}</span>}
            </div>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:18 }}>
          {CATEGORIES.map(c => (
            <button key={c.id} onClick={() => setCat(c.id)} style={{ padding:"6px 13px", borderRadius:20, border:`2px solid ${cat===c.id?c.color:"#eee"}`, background:cat===c.id?`${c.color}18`:"white", color:cat===c.id?c.color:"#aaa", fontSize:12, fontWeight:700, cursor:"pointer", transition:"all .2s" }}>{c.emoji} {c.label}</button>
          ))}
        </div>
        ...
        <div style={{ display:"flex", alignItems:"center", background:"#f8f8f8", borderRadius:16, padding:"0 18px", marginBottom:18 }}>
          ...
          <input type="number" value={amount} onChange={e => setAmt(e.target.value)} style={{ flex:1, border:"none", background:"none", padding:"14px 8px", fontSize:28, fontWeight:900, color:"#1a1a2e", outline:"none" }} />
        </div>
      ...
      <div style={{ flex:1, overflowY:"auto", overscrollBehavior:"contain", touchAction:"pan-y", padding:"18px 20px 40px", WebkitOverflowScrolling:"touch" }}>
        {step === 1 && (
          <>
            ...
            <input ref={fileRef} type="file" accept="image/*" onChange={handleFile} style={{ display:"none" }} />
            <button onClick={() => setStep(2)} style={{ width:"100%", marginTop:12, padding:14, borderRadius:16, border:"none", background:"#f5f5f5", color:"#bbb", fontSize:14, cursor:"pointer", fontWeight:700 }}>사진 없이 계속하기 →</button>
          </>
        )}
        {step === 2 && (
          <>
            ...
            <div style={{ display:"flex", gap:10, marginBottom:18 }}>
              <button onClick={() => setPay("cash")} style={{ flex:1, padding:"12px 0", borderRadius:16, border:`2px solid ${payMethod==="cash"?"#FF6B6B":"#eee"}`, background:payMethod==="cash"?"#fff0f0":"white", color:payMethod==="cash"?"#FF6B6B":"#aaa", fontSize:14, fontWeight:800, cursor:"pointer", transition:"all .2s" }}>💵 현금</button>
              <button onClick={() => setPay("card")} style={{ flex:1, padding:"12px 0", borderRadius:16, border:`2px solid ${payMethod==="card"?"#45B7D1":"#eee"}`, background:payMethod==="card"?"#f0f9ff":"white", color:payMethod==="card"?"#45B7D1":"#aaa", fontSize:14, fontWeight:800, cursor:"pointer", transition:"all .2s" }}>💳 카드</button>
            </div>
            <label style={{ display:"block", fontSize:11, fontWeight:800, color:"#bbb", letterSpacing:".06em", marginBottom:8 }}>카테고리</label>
            <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:18 }}>
              {CATEGORIES.map(cc => (
                <button key={cc.id} onClick={() => setCat(cc.id)} style={{ padding:"6px 13px", borderRadius:20, border:`2px solid ${cat === cc.id ? cc.color : "#eee"}`, background: cat === cc.id ? `${cc.color}18` : "white", color: cat === cc.id ? cc.color : "#aaa", fontSize:12, fontWeight:700, cursor:"pointer", transition:"all .2s" }}>{cc.emoji} {cc.label}</button>
              ))}
            </div>
            <label style={{ display:"block", fontSize:11, fontWeight:800, color:"#bbb", letterSpacing:".06em", marginBottom:8 }}>얼마 썼어요?</label>
            <div style={{ display:"flex", alignItems:"center", background:"#f8f8f8", borderRadius:16, padding:"0 18px", marginBottom:18 }}>
              <span style={{ fontSize:20, fontWeight:900, color:cc.color }}>₩</span>
              <input type="number" placeholder="0" value={amount} onChange={e => setAmt(e.target.value)} style={{ flex:1, border:"none", background:"none", padding:"14px 8px", fontSize:28, fontWeight:900, color:"#1a1a2e", outline:"none" }} />
            </div>
            ...
            <button onClick={() => setStep(3)} style={{ width:"100%", padding:16, borderRadius:18, border:"none", background:"linear-gradient(135deg,#FF6B6B,#FF8E53)", color:"white", fontSize:16, fontWeight:900, cursor:"pointer", boxShadow:"0 4px 16px rgba(255,107,107,.4)" }}>다음 →</button>
          </>
        )}
        {step === 3 && (
          <>
      ...
      <div style={{ background:"white", borderRadius:22, padding:22, marginBottom:16, boxShadow:"0 2px 16px rgba(0,0,0,.06)" }}>
        ...
        {income.length === 0
          ...
          : Object.values(income.reduce((acc, p) => {
              ...
              if (!acc[key]) acc[key] = { label: key, amount: 0, count: 0 };
              acc[key].amount += p.amount; acc[key].count += 1;
              return acc;
      ...
      <div style={{ background:"white", borderRadius:24, overflow:"hidden", width:"100%", maxWidth:400, maxHeight:"90vh", overflowY:"auto", overscrollBehavior:"contain", animation:"slideUp .3s ease" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding:"12px 16px 6px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            {post.type !== "income" && (
              <button onClick={() => { const n = !liked; setLiked(n); if(n) onLike(post.id); }} style={{ background:"none", border:"none", cursor:"pointer", fontSize:26, padding:0, transition:"transform .2s", transform: liked?"scale(1.2)":"scale(1)" }}>{liked ? "❤️" : "🤍"}</button>
            )}
            {post.sticker && <span style={{ fontSize:20 }}>{post.sticker}</span>}
    ...
    <div>
      ...
      <div style={{ background:"white", borderRadius:22, padding:"28px 22px 22px", marginBottom:16, boxShadow:"0 2px 16px rgba(0,0,0,.06)", textAlign:"center" }}>
        <div style={{ width:88, height:88, borderRadius:"50%", overflow:"hidden", margin:"0 auto 14px", boxShadow:"0 0 0 3px white,0 0 0 5px #FF6B6B" }}>
          {profile.avatarUrl ? <img src={profile.avatarUrl} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }} /> : <div style={{ width:"100%", height:"100%", background:"linear-gradient(135deg,#FF6B6B,#FF8E53)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:40 }}>🐰</div>}
        </div>
      ...
      <div style={{ background:"white", borderRadius:22, padding:16, boxShadow:"0 2px 16px rgba(0,0,0,.06)" }}>
        ...
        {posts.length === 0
          ? <div style={{ textAlign:"center", padding:"30px 0", color:"#ddd" }}><div style={{ fontSize:40 }}>📷</div><div style={{ marginTop:8, fontSize:14 }}>아직 기록이 없어요</div></div>
              ...
              {posts.map(p => {
                ...
                return (
                  <div key={p.id} onClick={() => setSelectedPost(p)} style={{ aspectRatio:"1/1", borderRadius:8, overflow:"hidden", background:`${c.color}22`, cursor:"pointer", position:"relative" }}>
                    {p.imageUrl ? <img src={p.imageUrl} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }} /> : <div style={{ width:"100%", height:"100%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:28 }}>{p.sticker || c.emoji}</div>}
                    {p.type === "income" && <div style={{ position:"absolute", top:4, right:4, background:"#2E7D32", borderRadius:8, padding:"2px 5px", fontSize:9, color:"white", fontWeight:800 }}>수입</div>}
                  </div>
                );
              })}
            </div>
      ...
      <div style={{ background:"white", width:"100%", maxWidth:430, margin:"0 auto", borderRadius:"28px 28px 0 0", padding:"20px 24px 48px", animation:"modalUp .35s cubic-bezier(.34,1.56,.64,1)" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", marginBottom:24, gap:12 }}>
          <div style={{ width:80, height:80, borderRadius:"50%", overflow:"hidden", boxShadow:"0 4px 16px rgba(255,107,107,.3)" }}>
            {avatar ? <img src={avatar} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }} /> : <div style={{ width:"100%", height:"100%", background:"linear-gradient(135deg,#FF6B6B,#FF8E53)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:32 }}>🐰</div>}
          </div>
          <div style={{ display:"flex", gap:8 }}>
            <button onClick={() => fileRef.current.click()} style={{ padding:"7px 16px", borderRadius:20, border:"2px solid #FF6B6B", background:"white", color:"#FF6B6B", fontSize:13, fontWeight:700, cursor:"pointer" }}>카메라</button>
            <button onClick={() => galleryRef.current.click()} style={{ padding:"7px 16px", borderRadius:20, border:"2px solid #FF6B6B", background:"white", color:"#FF6B6B", fontSize:13, fontWeight:700, cursor:"pointer" }}>앨범</button>
    ...
    <div style={{ maxWidth:430, margin:"0 auto", minHeight:"100vh", background:"#f4f4f8", fontFamily:"'Noto Sans KR',sans-serif", overflowX:"hidden", width:"100%" }}>
      ...
      <div style={{ position:"sticky", top:0, zIndex:100, background:"white", boxShadow:"0 2px 8px rgba(0,0,0,.06)", width:"100%" }}>
        <div style={{ padding:"12px 14px 8px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            <div style={{ fontSize:18, fontWeight:900, color:"#1a1a2e", letterSpacing:"-.5px" }}>💰 용돈일기</div>
        ...
        <div style={{ display:"flex", borderTop:"1px solid #f0f0f0" }}>
          {[["feed","🏠 홈"],["stats","📊 통계"],["profile","👤 프로필"]].map(([id, label]) => (
            <button key={id} onClick={() => setTab(id)} style={{ flex:1, padding:"10px 0", border:"none", background:"none", cursor:"pointer", fontSize:12, fontWeight:700, color: tab === id ? "#FF6B6B" : "#bbb", borderBottom: tab === id ? "2.5px solid #FF6B6B" : "2.5px solid transparent", transition:"all .2s" }}>
              {label}
            </button>
          ))}
        </div>
